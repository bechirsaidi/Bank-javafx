...

{
    public static boolean socialSecurityValid(String social)
    {
        ...
        if(!matcher.find())
        {
            Message.showMessage(Message.ERROR, "Message", "rib number can only contain digits");
            return false;
        }
        else if(social.length() != 9)
        {
            Message.showMessage(Message.ERROR, "Message", "rib number must be 9 digits long");
            return false;
        }
        return true;
    }
